...
#     while True:
#         yield (b'--frame\r\n'
#                b'Content-Type: image/jpeg\r\n\r\n' + sketch.response_img + b'\r\n\r\n')

# @app.route('/res_feed')
# def res_feed():
#     return Response(gen_res(), mimetype='multipart/x-mixed-replace; boundary=frame')

@app.route('/img_trans', methods=['POST'])
def img_trans():
    tp.start()
    ...
    img_np = cv2.imdecode(img_np, 1) # <class 'numpy.ndarray'> (480, 640, 3); flag=1, 8位深度，3通道
    # print(type(img_np), img_np.shape)
    # cv2.imwrite('base64.jpg',img_np)
    if len(img_np.shape) is not 3: # url_for('img_trans') may get this in first time
...
def image():
    tp.start()
    req_file = request.files['image']  # get the image
    ...
    tp.getCost('pre')
    wp = sketch.photo_G_1(img_in)
    img_out = sketch.sketch_G_2(wp)
    tp.getCost('net')
    img_out = img_out.cpu()
    img_res = img_out.data[0]
    saveImage(img_res, img_res_name, size=(sketch.IMG_W, sketch.IMG_H, 3))
    tp.getCost('post')
    tp.getTotalCost()

    with open(img_res_name, 'rb') as img_f:#读取图片文件
        bs64data = base64.b64encode(img_f.read()).decode()#进行base64编码
        html_str = "data:image/jpeg;base64,{}".format(bs64data) # 更新图片代码
        return html_str
